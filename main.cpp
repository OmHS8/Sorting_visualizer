...
public:
	...
	void draw(int color)  // Define function to draw a bar object with graphics' functions.  
	{   
...
void update(Bar arr[], int n)  // Define function to update the Bar chart 
{
	cleardevice();
	int x = 100;
	for (int i = 0; i<n; i++) {
...
void bubbleSort(Bar arr[], int n)  // Define sorting algorithm :- Bubble sort.
{
	int i, j;
	for (i = 0; i < n - 1; i++)
	{
		for (j = 0; j < n - i - 1; j++){
			if (arr[j].get_height() > arr[j + 1].get_height()) {   // Compare the heights of bar objects.
